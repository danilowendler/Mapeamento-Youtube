...
| T5 | "Vídeo que casou": em buscas por keyword/nicho, o chip do canal ganha subtítulo "entrou por: «título»" — o vídeo (`keyword_results.video_id`) que trouxe o canal para a pesquisa. Read-time (join com `videos`, título só quando já coletado — degrada em silêncio), zero cota, não muda ranking. `pickBestMatchPerChannel`/`matchedVideosByChannel` no keywordService (+6 testes). Só keyword/nicho (busca por canal = usuário escolheu). Nasceu da dúvida "minecraft world cup não traz vídeo da copa" — conecta a query ao resultado sem virar busca do YouTube | ✅ 20/07/2026 |
| T6 | Layout da página de resultados (feedback do fundador vendo prod): o resumo de canais sai do topo e vira a **aba "Canais"** (4ª aba no `ResultsView`, `f=canais`) — `ChannelOverview` em largura total (status · nº de oportunidades · "entrou por" legível · salvar), header enxuto. Aba **Trending** ganha destaque sutil: ponto vermelho fixo + sublinhado `primary` quando ativa (vermelho segue escasso — só acento, sem preenchimento). Estados vazios agora cientes de coleta ("Coletando…") e apontam a aba Canais quando não há outliers. `ChannelChips` removido | ✅ 20/07/2026 |
